...
fun MedicationScreen() {
    ...
    // Obtener medicamentos desde la base de datos en tiempo real
    ...
    var medType by rememberSaveable { mutableStateOf("Medicamento") } // "Medicamento" o "Insulina"
    ...
    LazyColumn(
        ...
        // --- ITEM: Añadir Medicamento/Insulina ---
        item {
            Text(
            ...
            Column(
                ...
                Row(
                    ...
                    val chipColors = FilterChipDefaults.filterChipColors(
                        ...
                        selectedContainerColor = Color(0xFF2196F3), // <-- Azul vivo
            ...
            if (medType != "Insulina") {
                OutlinedTextField(
                    ...
                    label = {
                        Text("Nombre del medicamento") // <-- Simplificado
                    },
                    modifier = Modifier.fillMaxWidth()
                )
                Spacer(modifier = Modifier.height(8.dp))
            ...
            // Campo de dosis diferente según el tipo
            OutlinedTextField(
                value = medDose,
            ...
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceEvenly) {
                ...
                Button(
                    ...
                    modifier = Modifier.weight(1f),
                    colors = ButtonDefaults.buttonColors(
                        containerColor = Color(0xFFF48FB1), // Rosa
                        contentColor = Color.Black          // Texto/Icono Negro
                    )
                ) {
            ...
            Button(
                onClick = {
                    ...
                    // Asignar nombre por defecto si es insulina
                    val nameToSave = if (isInsulin) "Insulina" else medName

                    // Validar: si es insulina, el nombre no importa. Si no, sí.
                    val isValid = medDose.isNotBlank() && selectedEndDate != null && (isInsulin || medName.isNotBlank())

                    if (isValid) {
                        val newMedication = Medication(
                            id = System.currentTimeMillis(),
                            name = nameToSave, // <-- Usar el nombre asignado
                            dose = medDose,
                            // --- ¡TE FALTABAN ESTOS CAMPOS! ---
                            hour = timeState.hour,
                        ...
                        medType = "Medicamento" // Resetear a valor por defecto
                    } else {
        ...
        // --- LISTA DE MEDICAMENTOS/INSULINA ---
                ...
                Column(modifier = Modifier.padding(16.dp)) {
                    ...
                    Row(
                        ...
                        Column(modifier = Modifier.weight(1f)) {
                            ...
                            Text(
                                text = if (med.type == "Insulina") "${med.dose} unidades"
                                else med.dose,
                    ...
                    Row(
                        ...
                        Button(
                            onClick = {
                                scope.launch {
                                    localStorageService.updateMedicationState(med.id, "TAKEN")
                                }
                                scheduler.cancel(med)
                            },
                        ...
                        Button(
                            ...
                            colors = ButtonDefaults.buttonColors(
                                ...
                                contentColor = MaterialTheme.colorScheme.onError // <-- Blanco
                            )
                        ) {
        ...
        confirmButton = {
            TextButton(
                onClick = {
                    // --- INICIA EL CAMBIO ---
                    val isInsulin = (medType == "Insulina")
                    // Asigna "Insulina" como nombre si ese es el tipo
                    val nameToSave = if (isInsulin) "Insulina" else medName

                    // La validación cambia: si es Insulina, no nos importa el medName.
                    val isValid = medDose.isNotBlank() && (isInsulin || medName.isNotBlank())

                    if (isValid) {
                        // --- FIN DEL CAMBIO ---
                        val updatedMedication = medication.copy(
                            name = nameToSave, // <-- Usar el nombre corregido
                            dose = medDose,
                            hour = timeState.hour,
                            minute = timeState.minute,
                            endDate = selectedEndDate,
